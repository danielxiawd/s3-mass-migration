...
def check_queue(qurl, dead_queue, sqs_profile=None, s3_profile=None):
    print('check_queue(%s, %s)'%(qurl, dead_queue))
    #return {'number':0}

    if sqs_profile is None or s3_profile is None:
        return {'number':0}

    # FIXME TODO later from config
    response = sqs_profile.sqs_client.receive_message(
        QueueUrl=qurl,
    ...
    for item in response['Messages']:
        ...
        for action in body:
            ...
            if s3_profile.s3_copy(src_bucket, dst_bucket, key):
                ...
                #success=False
                sqs_profile.send_msg_to_sqs(dead_queue, action)
                success=True
                break
        ...
        if success is True:
            response = sqs_profile.sqs_client.delete_message(
                QueueUrl=qurl,
...
def TaskExecutor(job_position=None):
    ...
    data = s3_profile.load_json_from_s3_object(bucket_name, key_name)
    ...
    while True:
        if check_point > q_number:
            check_point=1
        response = check_queue('%s-%03d'%(qurl_endpoint, check_point), dead_queue, sqs_profile=sqs_profile, s3_profile=s3_profile)
